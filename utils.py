...
def get_data(F, info):
    ...
    if info['no_of_categorical_features'] > 0:
        print(F['CAT'].dtypes)
        data = F['CAT'].fillna('nan').astype('|S').values if len(data) == 0 else \
                np.concatenate((data, F['CAT'].fillna('nan').astype('|S').values), axis=1)

    if info['no_of_mvc_features'] > 0:
        data = F['MV'].fillna('nan').astype('|S').values if len(data) == 0 else \
                np.concatenate((data, F['MV'].fillna('nan').astype('|S').values), axis=1)
